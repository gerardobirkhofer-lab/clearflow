...
async def get_dashboard_summary(
    ...
    """Get today's key metrics for the dashboard."""
    today = date.today()
    ...
    yesterday_query = select(func.sum(CardCollection.amount_gross)).where(
        and_(CardCollection.tenant_id == tenant_id, CardCollection.collection_date == yesterday)
    )
    yesterday_total = await db.scalar(yesterday_query) or 0

    # Cleared vs pending counts
    ...
    for row in status_counts:
        if row.status == ReconciliationStatus.CLEARED:
            cleared = row.total or 0
        elif row.status in (ReconciliationStatus.PENDING, ReconciliationStatus.PARTIAL):
