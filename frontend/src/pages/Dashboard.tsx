...
export default function Dashboard() {
  ...
  if (!report) return <div><h1>ClearFlow</h1><p>Loading report...</p></div>;
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, margin: '24px 0' }}>
        {[
          ...
          <div key={kpi.label} style={{ padding: 20, borderRadius: 8, background: '#fff', border: '1px solid #e2e8f0' }}>
      ...
      {uncleared.length === 0 ? <p style={{ color: '#64748b' }}>All clear.</p> : (
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8, background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          <thead><tr style={{ background: '#f1f5f9', textAlign: 'left' }}>
            <th style={{ padding: '12px 16px' }}>Institution</th><th style={{ padding: '12px 16px' }}>Count</th><th style={{ padding: '12px 16px' }}>Amount</th>
          </tr></thead>
          <tbody>{uncleared.map((inst, i) => (
            <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
              <td style={{ padding: '12px 16px' }}>{inst.name}</td>
              <td style={{ padding: '12px 16px' }}>{inst.uncleared_count}</td>
              <td style={{ padding: '12px 16px' }}>€{inst.uncleared_amount}</td>
            </tr>
          ))}</tbody>
        </table>
      )}
      ...
      {discrepancies.length === 0 ? <p style={{ color: '#64748b' }}>No discrepancies.</p> : (
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 8, background: '#fff', borderRadius: 8, overflow: 'hidden', border: '1px solid #e2e8f0' }}>
          <thead><tr style={{ background: '#f1f5f9', textAlign: 'left' }}>
            <th style={{ padding: '12px 16px' }}>Reference</th><th style={{ padding: '12px 16px' }}>Institution</th><th style={{ padding: '12px 16px' }}>Expected</th><th style={{ padding: '12px 16px' }}>Difference</th>
          </tr></thead>
          <tbody>{discrepancies.map((d, i) => (
            <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
              <td style={{ padding: '12px 16px' }}>{d.reference}</td>
              <td style={{ padding: '12px 16px' }}>{d.institution_name}</td>
              <td style={{ padding: '12px 16px' }}>€{d.expected_amount}</td>
              <td style={{ padding: '12px 16px', color: '#dc2626' }}>€{d.difference}</td>
            </tr>
          ))}</tbody>
        </table>
      )}
